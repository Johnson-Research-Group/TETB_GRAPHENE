    ...
    for i in [-1, 0, 1]:
        for j in [-1, 0, 1]:
            for k in [-1, 0, 1]:
                ...
                RIJ = r2 + lp.dot(pbc, cell) - r1
                norm_RIJ = lp.linalg.norm(RIJ, axis=1)

                mask = norm_RIJ < d
                d = lp.where(mask, norm_RIJ, d)
                drij = lp.where(mask[:, None], RIJ, drij)
...
def gen_ham_ovrlp(atom_positions, atom_types, cell, kpoint, params):
    ...
    Ham = lp.zeros((natoms, natoms), dtype=lp.complex64)
    atom_types = np.array(atom_types)
    #upper triangle indices
    i_indices, j_indices = lp.triu_indices(natoms, k=0)
    disp = wrap_disp(atom_positions[i_indices], atom_positions[j_indices], cell)
    dist = lp.linalg.norm(disp, axis=1)
    self_energies = lp.ones(natoms)
    ...
    for i,typei in enumerate(atom_types_list):
        for j,typej in enumerate(atom_types_list):
            ...
            #print(typei,typej)
            valid_indices &= lp.array(atom_types[i_indices.get()]==typei)
            valid_indices &= lp.array(atom_types[j_indices.get()]==typej)

            phase = lp.exp(1j * lp.dot(kpoint, disp[valid_indices].T))
    ...
    plt.imshow(lp.asnumpy(Ham).real)
    plt.colorbar()
...
def get_hellman_feynman_fd(atom_positions, atom_types, cell, eigvec, kpoint, params):
    ...
    for dir_ind in range(3):
        ...
        atom_positions_pert[:, dir_ind] = lp.copy(atom_positions[:, dir_ind])
...
def get_hellman_feynman(atom_positions, atom_types, cell, eigvec, kpoint, params):
    ...
    Force = lp.zeros((natoms, 3), dtype=lp.complex64)
    i_indices, j_indices = lp.triu_indices(natoms, k=1)
    r1 = atom_positions[i_indices]
    r2 = atom_positions[j_indices]
    Nocc_mat = 2*lp.ones((natoms,natoms))
    Nocc_mat[:nocc,:nocc] = 0
    rho = lp.conj(eigvec[:, :nocc]).T @ Nocc_mat @eigvec[:, :nocc]
    ...
    for i,typei in enumerate(atom_types_list):
        ...
        for j,typej in enumerate(atom_types_list):        
            ...
            for i_curr,r_current in enumerate(r1_list):
                disp = wrap_disp(r_current, r2_list, cell)
                dist = lp.linalg.norm(disp, axis=1)
                rcut = params[typei][typej]["rcut"]
                rcut = lp.asarray(rcut)
                ...
                gradH = grad(helem_fxn(r1_list))
                #wrap_disp(r_current, r2[valid_indices], cell)
                phase = lp.exp(1j * lp.dot(kpoint, disp[valid_indices,:]))

                ave_gradH = -lp.sum(4*gradH*rho[i_indices,j_indices]*phase)
                Force[i_curr,:] = ave_gradH
...
def get_hellman_feynman(atom_positions, atom_types, cell, eigvec, kpoint, params):
    ...
    # Use atom_types directly for indexing
    for i,typei in enumerate(atom_types_list):
        type1_indices = lp.array(np.squeeze(np.where(atom_types==typei)))
        for j,typej in enumerate(atom_types_list):
            type2_indices = lp.array(np.squeeze(np.where(atom_types==typej)))
            dr = 1e-3
            for i_ind in type1_indices:
                ...
                for j_ind in type2_indices:
                    ...
                    disp = wrap_disp(r1, r2, cell)
                    dist = lp.linalg.norm(disp)
                    rcut = params[typei][typej]["rcut"]
                    rcut = lp.asarray(rcut)
                    if dist > rcut:
                        continue

                    ave_gradH = lp.zeros((3))
                    disp = wrap_disp(r1, r2, cell)
                    phase = lp.exp(1j * lp.dot(kpoint, disp))
                    
                    for i in range(3):
                        dr_v = lp.zeros_like(r1)
                        dr_v[i]+=dr
                        disp = wrap_disp(r1-dr_v, r2, cell)
                        H_d = params[typei][typej]["hopping"](lp.array([disp]))

                        disp = wrap_disp(r1+dr_v, r2, cell)
                        H_up = params[typei][typej]["hopping"](lp.array([disp]))
                        gradH = (H_up - H_d)/2/dr

                        rho = lp.dot(eigvec[j_ind,:nocc],lp.conj(eigvec[i_ind,:nocc]))

                        ave_gradH = -lp.sum(4 * gradH * rho * phase)
