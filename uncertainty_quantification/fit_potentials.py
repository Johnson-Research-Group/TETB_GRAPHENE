...
def write_kcinsp(params,kc_file):
    ...
    params = params[:9]
    params = " ".join([str(x) for x in params]).replace("[","").replace("]","")
...
class fit_potentials_tblg:
    def __init__(self,calc_obj, db, potential,optimizer_type="Nelder-Mead",fit_forces=False):
        ...
        if self.potential=="rebo":
            ...
            self.output = "rebo"
            self.potential_file = os.path.join(self.calc.output,self.calc.rebo_file)
        elif self.potential=="KC inspired":
            self.write_potential = write_kcinsp
            self.output = "KCinsp"
            self.potential_file = os.path.join(self.calc.output,self.calc.kc_file)
        
    def objective(self,params,E0="last"):
        energy = []
        rms=[]
        fit_energy = []
        tb_energy  =[]
        ...
        if self.optimizer_type=="global":
            use_params = self.original_p0
            use_params[self.fit_param] = params
            self.write_potential(use_params,self.potential_file)
        else:
        ...
        for row in self.db.select():
            ...
            if self.output=="KCinsp":
                ...
                if np.abs(np.mean(bot_pos[:,2])-np.mean(top_pos[:,2])) < 3.15:
                    continue
            ...
            #    continue
            #if np.abs((e-(row.data.total_energy))) > 0.1:
            #    continue
            ...
            if self.output=="rebo":
                plot_axis.append(average_distance)
                if np.abs((e-(row.data.total_energy))) > 0.1:
                    continue
            elif self.output=="KCinsp":
                mean_z = np.mean(pos[:,2])
                top_layer_ind = np.where(pos[:,2]>mean_z)
                top_pos = np.squeeze(pos[top_layer_ind,:])
                bot_layer_ind = np.where(pos[:,2]<mean_z)
                bot_pos = np.squeeze(pos[bot_layer_ind,:])
                plot_axis.append(np.abs(np.mean(bot_pos[:,2])-np.mean(top_pos[:,2])))

        #print(energy)
        if E0=="last":
             
            energy = np.array(energy) - energy[-1]
            fit_energy = np.array(fit_energy) - fit_energy[-1]
            tb_energy = np.array(tb_energy)
            tb_energy -= tb_energy[-1]
        else:
            min_ind = np.argmin(fit_energy)
            energy = np.array(energy) - np.min(energy)
            fit_energy = np.array(fit_energy) - np.min(fit_energy) 
            tb_energy = np.array(tb_energy)
            tb_energy -= tb_energy[min_ind]
        rms = np.linalg.norm(energy-fit_energy)
        plt.scatter(plot_axis,energy,label="model")
        plt.scatter(plot_axis,fit_energy,label="ab initio data")
        #plt.scatter(plot_axis,tb_energy,label="tb energy")
        plt.legend()
        plt.title("RMS = "+str(rms/len(atoms)))
        plt.savefig(os.path.join(self.calc.output,self.output+"plot.png"))
        plt.clf()
        #exit()
    ...
    def fit(self,p0,bounds=None):
        ...
        if self.optimizer_type=="Nelder-Mead":
            ...
            self.fit_param = np.arange(0,len(params),1)
...
if __name__ == '__main__':
    ...
    intralayer_potential = np.load("mnml_hopping_rebo_residual_params.npz")["params"]
    interlayer_potential = np.load("mnml_hopping_kcinsp_residual_params.npz")["params"]
    interlayer_potential = np.array([0.719345289329483, 6.074935002291668, 18.184672181803677,
             13.394207130830571, 0.003559135312169, 3.379423382381699,
             -0.25489099123815506, -0.0476875, -0.0491875]) 
    ...
    if args.gendata=="True" and args.type=="interlayer":
        ...
        subprocess.call('rm ../data/mnml_hopping_bilayer_nkp'+nkp+'.db',shell=True)
        db = ase.db.connect('../data/mnml_hopping_bilayer_nkp'+nkp+'.db')
    ...
    if args.type=="interlayer" and args.fit=="True":
        ...
        params = pfinal.x
        np.savez("mnml_hopping_kcinsp_residual_params",params=params)
        print(pfinal.x)

    if args.gendata=="True" and args.type=="intralayer":
        calc_obj = TETB_GRAPHENE_Calc(model_dict,use_overlap=False)
        subprocess.call('rm ../data/mnml_hopping_monolayer_nkp'+nkp+'.db',shell=True)
        db = ase.db.connect('../data/mnml_hopping_monolayer_nkp'+nkp+'.db')
        dft_db = ase.db.connect('../data/monolayer_nkp'+nkp+'.db')
        for row in dft_db.select():
            atoms = dft_db.get_atoms(id = row.id)
            atoms.set_array("mol-id",np.ones(len(atoms),dtype=np.int64))
            atoms.calc = calc_obj
            tb_energy,tb_forces = calc_obj.run_tight_binding(atoms)
            total_energy = row.data.total_energy
            db.write(atoms,data={"total_energy":total_energy,'tb_forces':tb_forces,'tb_energy':tb_energy/len(atoms)})

    old_gen=False
    if args.gendata=="True" and args.type=="intralayer" and old_gen:
        calc_obj = TETB_GRAPHENE_Calc(model_dict,use_overlap=False)
        print("assembling intralayer database")
        subprocess.call('rm ../data/mnml_hopping_monolayer_nkp'+nkp+'.db',shell=True)
        db = ase.db.connect('../data/mnml_hopping_monolayer_nkp'+nkp+'.db')
        file_list = glob.glob("/projects/bcmp/dpalmer3/tBLG_DFT/grapheneCalc*",recursive=True)
    ...
    if args.type=="intralayer" and args.fit=="True":
        ...
        p0 = [0.14687637217609084, 4.683462616941604, 12433.64356176609,\
        ...
        params = pfinal.x
        np.savez("mnml_hopping_rebo_residual_params",params=params)
        print(pfinal.x)
    ...
    if args.type=="intralayer" and args.test=="True":
        ...
        calc_obj_classical = TETB_GRAPHENE_Calc(model_dict)
        # 'Q_CC' ,'alpha_CC', 'A_CC'
        #'BIJc_CC1', 'BIJc_CC2','BIJc_CC3',
        #'Beta_CC1', 'Beta_CC2', 'Beta_CC3
        #p0 = [0.3134602960833/1.2, 4.7465390606595, 10953.544162170,\
