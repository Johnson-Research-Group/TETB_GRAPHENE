...
void PairRegDepPoly::init_style()
{
  ...
  //neighbor->requests[irequest]->ghost = 1;
   neighbor->find_request(this, NeighConst::REQ_FULL | NeighConst::REQ_GHOST);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype] && atom->molecule[i] != atom->molecule[j]) {
        ...
        sumP = -1*(p.A6*p.z06*r6inv + p.A8*p.z08*r8inv + p.A10*p.z010*r10inv); // -C6*(r/z0)^-6 - C8*(r/z0)^-8 - C10*(r/z0)^-10
        ...
        sumC11 = (p.C2 + 2.0*p.C4*rho_ij_by_delta_sq)*p.delta2inv; //derivative of the term above with respect to (rho_ij)^2 (BUT NOT THE rho_ij IN KOLMOGOROV_KRESPI_FULL)
        frho_ij = expf*sumC1;
        sumCff = 0.5*p.C + frho_ij; //"half" of the registry dependent term [C+f(rho_ij)+f(rho_ji)]
        Vkc = sumCff*sumP; //potential energy

        // in the reference file fpair  = -(1/r) * (d exp0/dr) * sumCff
        //                       fpair1 = -(d rhoij/dr)^{-1} * (d sumCff/dr) * exp0
        // so we expect that here: fpair  = -(1/r) * sumCff * (d sumP/dr)
        //                         fpair1 = -(d rhhhoij/dr)^{-1} * (d sumCff/dr) * sumP
        // since our potential has the same product structure

        fpair = -(6.0*p.A6*p.z06*r8inv + 8.0*p.A8*p.z08*r10inv + 10.0*p.A10*p.z010*r12inv)*sumCff;
        fpair1 = 2.0*expf*(p.delta2inv*sumC1 - sumC11)*sumP;
        fsum = fpair + fpair1;

        // TODO: after everything appears in order, change some names to make more sense (Vkc->VvdW)
        ...
        f[j][0] -= fkcx + fprod1[0]*Tap;
        f[j][1] -= fkcy + fprod1[1]*Tap;
        f[j][2] -= fkcz + fprod1[2]*Tap;
	...
	//	  ev_tally(i, j, nlocal, newton_pair, evdwl, 0.0, fsum, delx, dely, delz);
      ...
      // TODO: determine proper way to avoid offsets from LAMMPS script
