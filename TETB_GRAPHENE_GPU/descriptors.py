...
def nnmat(lattice_vectors, atomic_basis):
    ...
    """
    nnmat = cp.zeros((len(atomic_basis), 3, 3))

    # Extend atom list
    atoms = []
    for i in [0, -1, 1]:
        for j in [0, -1, 1]:
            displaced_atoms = atomic_basis + lattice_vectors[cp.newaxis, 0] * i + lattice_vectors[cp.newaxis, 1] * j
            atoms += [list(x) for x in displaced_atoms]
    atoms = cp.array(atoms)
    atomic_basis = cp.array(atomic_basis)

    # Loop
    for i in range(len(atomic_basis)):
        displacements = atoms - atomic_basis[i]
        distances = cp.linalg.norm(displacements,axis=1)
        ind = cp.argsort(distances)
...
def ix_to_dist(lattice_vectors, atomic_basis, di, dj, ai, aj):
    ...
    displacement_vector = di[:, cp.newaxis] * lattice_vectors[0] +\
    ...
    dxy = cp.linalg.norm(displacement_vector_xy, axis = 1)
    dz = cp.abs(displacement_vector_z)
    return dxy, dz
#@njit
...
def partition_tb(lattice_vectors, atomic_basis, di, dj, ai, aj):
    ...
    distances = cp.sqrt(distances[0]**2 + distances[1]**2)
    ...
    t02_ix = (distances >= 0.95 * cp.sqrt(3) * min_distance) & (distances <= 1.05 * cp.sqrt(3) * min_distance)
...
def t02_descriptors(lattice_vectors, atomic_basis, di, dj, ai, aj):
    ...
    for i in range(len(r)):
        ...
        nndist = cp.linalg.norm(nn, axis = 1)
        ind = cp.argsort(nndist)
...
def ix_to_orientation(lattice_vectors, atomic_basis, di, dj, ai, aj):
    ...
    for disp, i, j, inn, jnn in zip(displacement_vector, ai, aj, mat[ai], mat[aj]):
        sin_jnn = cp.cross(jnn[:,:2], disp[:2]) 
        sin_inn = cp.cross(inn[:,:2], disp[:2]) 
        cos_jnn = cp.dot(jnn[:,:2], disp[:2]) 
        cos_inn = cp.dot(inn[:,:2], disp[:2]) 
        theta_jnn = cp.arctan2(sin_jnn, cos_jnn)
        theta_inn = cp.arctan2(sin_inn, cos_inn)

        theta_12.append(cp.pi - theta_jnn[0])
...
def descriptors(lattice_vectors, atomic_basis, di, dj, ai, aj):
    ...
    dist = cp.sqrt(dist_z ** 2 + dist_xy ** 2)
