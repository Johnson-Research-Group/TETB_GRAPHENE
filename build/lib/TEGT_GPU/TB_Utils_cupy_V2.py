...
def get_hellman_feynman(atomic_basis, layer_types, lattice_vectors, eigvec, model_type, indi,indj, di, dj, phases):
    ...
    nocc = natoms//2
    fd_dist = cp.eye(natoms)
    fd_dist[nocc:,nocc:] = 0
    density_matrix = cp.conj(eigvec) @ fd_dist @ eigvec
    ...
    for i in range(natoms):
        ...
        for j in range(3):
            ...
            h_up = hopping_model(lattice_vectors, atomic_basis,indi[neigh_ind_i], indj[neigh_ind_i], di[neigh_ind_i], dj[neigh_ind_i], layer_types=layer_types) / 2 # this should be 1d array size(neigh_ind_i)
            #repeat in opposite direction
            perturb_pos = atomic_basis
            perturb_pos[i,j] -=dr
            #compute hoppings of its neighbors
            h_d = hopping_model(lattice_vectors, atomic_basis,indi[neigh_ind_i], indi[neigh_ind_i], di[neigh_ind_i], dj[neigh_ind_i], layer_types=layer_types) / 2
